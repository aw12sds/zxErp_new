	...
	*/
	...
	public int updateLcProcessStatus(String lc_process_id,Map<String,Object> condition){
		...
		try {
			...
			String attachPath = System.getProperty("java.io.tmpdir");
			if(lc_Process.getLc_process_flag() == 0){
				...
				FtpUtil.downFile(getXtPathCache("ActivitiLcFtp").get(0).getXt_path(), attachment.getXt_attachmentName(), attachPath);
				attachPath = FileUtil.validOrCreateFile(attachPath+attachment.getXt_attachmentName());
			}
			...
			if(!file.exists()){
				...
				if(lc_Process.getLc_process_flag() == 1){
					FileUtil.delFile(attachPath);
				}
			}
