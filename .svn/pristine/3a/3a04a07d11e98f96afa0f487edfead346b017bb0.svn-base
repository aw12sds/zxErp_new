...
public class ZxSixSController extends BaseAction{
	...
	*/
	...
	public String getZxSixSListViewByCondition(BaseSearch baseSearch,HttpServletRequest request){
		...
		condition.put("is_connect", 1);
		condition.put("state", 1);
		if(StringUtils.isNotBlank(request.getParameter("statuss"))){
			condition.put("statuss", request.getParameter("statuss").split(","));
		}
	...
	*/
	...
	public String addZxSixS(ZxSixS zxSixS,HttpServletRequest request){
		int i = 0;
		String id=null;
		if(null != zxSixS&&StringUtils.isNotBlank(zxSixS.getTitle())){
			Map<String, Object> condition = new HashMap<String, Object>();
			condition.put("e_title", zxSixS.getTitle());
			List<ZxSixS> list = zxSixSService.getZxSixSListByCondition(condition);
			if(list.isEmpty()){
				id = UUID.toUUID();
				zxSixS.setId(id);
				...
				return "{success:false,msg:'信息已存在',responseFlag:false}";
			}
		}
		if(i>0){
	...
	*/
	...
	public String updateZxSixS(ZxSixS zxSixS,HttpServletRequest request){
		int i = 0;
		if(null != zxSixS&&StringUtils.isNotBlank(zxSixS.getTitle())&&zxSixS.getStatus().equals("1")){
			Map<String, Object> condition = new HashMap<String, Object>();
			condition.put("e_title", zxSixS.getTitle());
			List<ZxSixS> list = zxSixSService.getZxSixSListByCondition(condition);
			if(list.isEmpty()||(list.size() == 1&&list.get(0).getId().equals(zxSixS.getId()))){
		...
		if(null != zxSixS&&zxSixS.getState().equals("1")&&zxSixS.getStatus().equals("1")){
			...
			if(!list.isEmpty()){
				...
				if(i > 0&&zxSixS.getStatus().equals("2")){
					...
					if(!messages.isEmpty()){
						i = xtMessageService.addBatchXtMessage(messages);
					}
				}
			}
		}
	...
	* @param request 
	*/
	@RequestMapping(value="/toZxSixSAdd",method={RequestMethod.POST,RequestMethod.GET})
	public ModelAndView toZxSixSAdd(ZxSixS zxSixS,HttpServletRequest request){
		return new ModelAndView("pc/zx-view/zx-six-s/zx-six-s-add");
	}
	/**
	* 发送至编辑页面
	* @param request 
	*/
	@RequestMapping(value="/toZxSixSUpdate",method={RequestMethod.POST,RequestMethod.GET})
	public ModelAndView toZxSixSUpdate(String id,HttpServletRequest request, Model model){
		ZxSixS zxSixS = zxSixSService.getZxSixSById(id);
		model.addAttribute("zxSixS", zxSixS);
		return new ModelAndView("pc/zx-view/zx-six-s/zx-six-s-update");
	}
	/**
	* 发送至明细页面
	* @param request 
	*/
	@RequestMapping(value="/toZxSixSDetail",method={RequestMethod.POST,RequestMethod.GET})
	public ModelAndView toZxSixSDetail(String id,HttpServletRequest request, Model model){
		ZxSixS zxSixS = zxSixSService.getZxSixSById(id);
		model.addAttribute("zxSixS", zxSixS);
		return new ModelAndView("pc/zx-view/zx-six-s/zx-six-s-detail");
	}
}
