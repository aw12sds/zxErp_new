	...
	public static String buildTree(List<BaseTreeEntity> list){  
        ...
        for(int i = 0; i < list.size(); i++) { 
            ...
            if ("0".equals(node.getPid())) {  
            	...
            	html.append("{\"text\":\"" + node.getText() + "\",\"id\":\"" + node.getId()+ "\",\"checked\":"+node.getChecked()+",\"singleClickExpand\":"+node.getSingleClickExpand()+",\"pid\":false,\"leaf\":"+node.getLeaf()+",\"icon\":\""+node.getIcon()+"\",\"qtip\":\""+node.getText()+"\",\"expanded\":"+node.getExpanded());
