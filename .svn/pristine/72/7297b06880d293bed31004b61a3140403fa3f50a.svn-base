...
function submit(status){
	msgTishCallFnBoot("确定要提交该表单信息？",function(){
		...
		var param = {status:status,scores:scores.join("-"),supplier_id:$('#supplier_id').val(),months:$('#months').val()};
		$.ajax({
...
$(document).ready(function(){
	...
	$(".content-table").each(function(){
	    ...
	    var dic_id = $(this).attr('data-id');
	    $.post("../zxJudgeReportController/getZxJudgeAgainReportTableColumnList",{dic_id:dic_id,months:$('#months').val(),supplier_id:$('#supplier_id').val()},function(result){
