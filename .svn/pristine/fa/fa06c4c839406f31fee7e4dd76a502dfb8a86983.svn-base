...
public class ZxJudgeReportController extends BaseAction{
	...
	*/
	...
	public String getYCStep(String supplier_id,Integer num,HttpServletRequest request){
		...
		if(!CommonUtils.isAdmin()&&sdfURL.contains(",zxJudgeReportController/saveReport,")){
			Map<String, Object> condition = new HashMap<String, Object>();
			condition.put("supplier_id", supplier_id);
	...
	public String getAgainStep(String supplier_id,HttpServletRequest request){
		int i = 0;
		String sdfURL = (String)request.getSession(false).getAttribute(SessionConstant.XT_FUNCTIONINFOURL);
		if(!CommonUtils.isAdmin()&&sdfURL.contains(",zxJudgeReportController/saveAgainReport,")){
	...
	*/
	...
	public String saveReport(String scores,String supplier_id,String apply_id,String status,HttpServletRequest request){
		...
		condition.put("scores", scores.split("-"));
	...
	* @param request 
	*/
	@RequestMapping(value="/toZxJudgeReportAdd",method={RequestMethod.POST,RequestMethod.GET})
	public ModelAndView toZxJudgeReportAdd(String supplier_id,String step,HttpServletRequest request, Model model){
		Map<String, Object> condition = new HashMap<String, Object>();
		condition.put("supplier_id", supplier_id);
		condition.put("state", 1);
		List<ZxJudgeApply> applys = zxJudgeApplyService.getZxJudgeApplyListByCondition(condition);
		if (applys != null && !applys.isEmpty()) {
			ZxJudgeApply apply = applys.get(0);
			model.addAttribute("judge_date_start", apply.getJudge_date_start());
			model.addAttribute("judge_date_end", apply.getJudge_date_end());
			model.addAttribute("apply_id", apply.getId());
			...
			if (results != null && !results.isEmpty()) {
				...
				model.addAttribute("result_id", result.getId());
				if(result.getStatus().endsWith("1"))
					model.addAttribute("status", "保存中");
				else if(result.getStatus().endsWith("2"))
					model.addAttribute("status", "审核中");
				else if(result.getStatus().endsWith("3"))
					model.addAttribute("status", "审核通过");
				else
					model.addAttribute("status", "审核退回");
			}
		}
		...
		model.addAttribute("standards", set);
		
		model.addAttribute("step", step);
		model.addAttribute("supplier_id", supplier_id);
		return new ModelAndView("pc/zx-view/zx-judge-result/zx-judge-report-add");
	}
	/**
	* 发送至新增页面
	* @param request 
	*/
	@RequestMapping(value="/toZxJudgeAgainReportAdd",method={RequestMethod.POST,RequestMethod.GET})
	public ModelAndView toZxJudgeAgainReportAdd(String supplier_id,String step,HttpServletRequest request, Model model){
		Map<String, Object> condition = new HashMap<String, Object>();
		condition.put("supplier_id",supplier_id);
		condition.put("state",1);
		condition.put("order_by_judge_date",1);
		...
		if(results != null&&!results.isEmpty()){
			...
			model.addAttribute("judge_date", months%12==0?(months/12 - 1) + "-12":months/12 + "-" + (months%12 < 10?"0" + months%12:months%12));
		...
		model.addAttribute("step", step);
		model.addAttribute("supplier_id", supplier_id);
		return new ModelAndView("pc/zx-view/zx-judge-result/zx-judge-again-report-add");
	}
}
