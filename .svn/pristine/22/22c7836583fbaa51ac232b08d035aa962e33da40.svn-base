...
$(document).ready(function() {
	...
	var options = DataTablesPaging.pagingOptions({
		ajax:function (data, callback, settings){datatablesCallBack(data, callback, settings,'../xtUserinfoController/getXtUserinfoListBootByCondition',opt);},//渲染数据
			//在第一位置追加序列号
		...
		colums:[
			...
			},
			{
				...
				render:function(data, type, row, meta) {
					return InitBDataCallFnByKey("gender",data);
				}
			},
			{
				data:'xt_userinfo_state',
				render:function(data, type, row, meta) {
					return InitBDataCallFnByKey("xt_userinfo_state",data);
				}
			},
			{
				data:"xt_userinfo_id",
				render:function(data, type, row, meta) {
					...
					var btn = '<a href="javascript:toXtUserinfoDetail(\''+data+'\')"><i class="glyphicon glyphicon-eye-open" title="详情"></i></a>&emsp;';
						btn = btn +'<a href="javascript:initRoleinfo(\''+data+'\')"><i class="glyphicon glyphicon-check text-warning" title="角色权限"></i></a>&emsp;';
						btn = btn +'<a href="javascript:resetXtUserinfoPwd(\''+data+'\',\''+xt_userinfo_realName+'\',\''+xt_userinfo_name+'\')"><i class="glyphicon glyphicon-edit text-danger" title="重置密码"></i></a>&emsp;';
					return btn;
				}
			}
		]
