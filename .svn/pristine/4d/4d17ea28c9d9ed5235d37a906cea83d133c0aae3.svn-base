	  ...
	  public static String buildTree(List<BaseTreeGridEntity> list,boolean isHasChecbox){  
        ...
        for(int i = 0; i < list.size(); i++) { 
            ...
            if ("0".equals(node.getPid())) {  
            	...
            	if(isHasChecbox){
            		html.append("{\"text\":\"" + node.getText() + "\",\"id\":\"" + node.getId()+ "\",\"checked\":"+node.getChecked()+",\"singleClickExpand\":"+node.getSingleClickExpand()+",\"pid\":false,\"leaf\":"+isLeaf(list,node)+",\"icon\":\""+node.getIcon()+"\",\"qtip\":\""+node.getText()+"\",\"expanded\":"+node.getExpanded()+",\"tempObject\":\""+node.getTempObject()+"\",\"content\":\""+node.getContent()+"\",integerappend:\""+node.getIntegerappend()+"\"");
            	}else{
            		html.append("{\"text\":\"" + node.getText() + "\",\"id\":\"" + node.getId()+ "\",\"singleClickExpand\":"+node.getSingleClickExpand()+",\"pid\":false,\"leaf\":"+isLeaf(list,node)+",\"icon\":\""+node.getIcon()+"\",\"qtip\":\""+node.getText()+"\",\"expanded\":"+node.getExpanded()+",\"tempObject\":\""+node.getTempObject()+"\",\"content\":\""+node.getContent()+"\",\"integerappend\":\""+node.getIntegerappend()+"\"");
            	}
