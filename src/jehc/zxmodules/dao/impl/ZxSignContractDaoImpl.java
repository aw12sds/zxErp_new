package jehc.zxmodules.dao.impl;
import java.util.List;
import java.util.Map;
import org.springframework.stereotype.Repository;
import jehc.xtmodules.xtcore.base.impl.BaseDaoImpl;
import jehc.zxmodules.dao.ZxSignContractDao;
import jehc.zxmodules.model.ZxSignContract;

/**
* 签订合同 
* 2018-01-11 13:56:25  owen
*/
@Repository("zxSignContractDao")
public class ZxSignContractDaoImpl  extends BaseDaoImpl implements ZxSignContractDao{
	/**
	* 分页
	* @param condition 
	* @return
	*/
	@SuppressWarnings("unchecked")
	public List<ZxSignContract> getZxSignContractListByCondition(Map<String,Object> condition){
		return (List<ZxSignContract>)this.getList("getZxSignContractListByCondition",condition);
	}
	/**
	* 查询对象
	* @param id 
	* @return
	*/
	public ZxSignContract getZxSignContractById(String id){
		return (ZxSignContract)this.get("getZxSignContractById", id);
	}
	/**
	* 添加
	* @param zx_sign_contract 
	* @return
	*/
	public int addZxSignContract(ZxSignContract zxSignContract){
		return this.add("addZxSignContract", zxSignContract);
	}
	/**
	* 修改
	* @param zx_sign_contract 
	* @return
	*/
	public int updateZxSignContract(ZxSignContract zxSignContract){
		return this.update("updateZxSignContract", zxSignContract);
	}
	/**
	* 修改（根据动态条件）
	* @param zx_sign_contract 
	* @return
	*/
	public int updateZxSignContractBySelective(ZxSignContract zxSignContract){
		return this.update("updateZxSignContractBySelective", zxSignContract);
	}
	/**
	* 删除
	* @param condition 
	* @return
	*/
	public int delZxSignContract(Map<String,Object> condition){
		return this.del("delZxSignContract", condition);
	}
	/**
	* 批量添加
	* @param zx_sign_contractList 
	* @return
	*/
	public int addBatchZxSignContract(List<ZxSignContract> zxSignContractList){
		return this.add("addBatchZxSignContract", zxSignContractList);
	}
	/**
	* 批量修改
	* @param zx_sign_contractList 
	* @return
	*/
	public int updateBatchZxSignContract(List<ZxSignContract> zxSignContractList){
		return this.update("updateBatchZxSignContract", zxSignContractList);
	}
	/**
	* 批量修改（根据动态条件）
	* @param zx_sign_contractList 
	* @return
	*/
	public int updateBatchZxSignContractBySelective(List<ZxSignContract> zxSignContractList){
		return this.update("updateBatchZxSignContractBySelective", zxSignContractList);
	}
}
