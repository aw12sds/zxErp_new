...
public class ZxApplyPhoneCardController extends BaseAction{
	...
	*/
	...
	public String getZxApplyPhoneCardListByConditionHR(BaseSearch baseSearch,HttpServletRequest request){
		...
		for (ZxApplyPhoneCard loan : zxApplyPhoneCardList) {
			...
			loan.setXt_user_name(phoneApply.getXt_userinfo_realName());
		}
		PageInfo<ZxApplyPhoneCard> page = new PageInfo<ZxApplyPhoneCard>(zxApplyPhoneCardList);
		return outPageBootStr(page,request);
	}
		...
		if(null != zxApplyPhoneCard && !"".equals(zxApplyPhoneCard)){
			...
			if(xtUserinfo.getXt_userinfo_ortherTel().equals("")){
				...
				if(list.isEmpty()&&list.size()==0){
					...
					i = 0;
					return "{success:false,msg:'你已创建申请',responseFlag:false}";
				}
			}else{
				i = 0;
		...
		if(null != task_id && !"".equals(task_id)){
			...
			if(activitiUtil.completeTask(task_id, variable)){
				...
				if(task_status.equals("yes")){
					...
					boolean isEnd = activitiUtil.validatePEnd(((Task) taskData.get("task")).getProcessInstanceId());
					if(isEnd){
	...
	* @param request 
	*/
	@RequestMapping(value="/toZxApplyPhoneCardDetail",method={RequestMethod.POST,RequestMethod.GET})
	public ModelAndView toZxApplyPhoneCardDetail(String id,HttpServletRequest request, Model model){
		ZxApplyPhoneCard zxApplyPhoneCard = zxApplyPhoneCardService.getZxApplyPhoneCardById(id);
		XtUserinfo phoneApply = xtUserinfoService.getXtUserinfoById(zxApplyPhoneCard.getApply_user_id());
		zxApplyPhoneCard.setXt_user_name(phoneApply.getXt_userinfo_realName());
		model.addAttribute("zxApplyPhoneCard", zxApplyPhoneCard);
		return new ModelAndView("pc/zx-view/zx-apply-phone-card/zx-apply-phone-card-detail");
	}
	/**
	* 发送至发放页面
	* @param request 
	*/
	@RequestMapping(value="/toGrantZxApplyPhoneCard",method={RequestMethod.POST,RequestMethod.GET})
	public ModelAndView toGrantZxApplyPhoneCard(String id,HttpServletRequest request, Model model){
		ZxApplyPhoneCard zxApplyPhoneCard = zxApplyPhoneCardService.getZxApplyPhoneCardById(id);
		XtUserinfo phoneApply = xtUserinfoService.getXtUserinfoById(zxApplyPhoneCard.getApply_user_id());
		zxApplyPhoneCard.setXt_user_name(phoneApply.getXt_userinfo_realName());
		model.addAttribute("zxApplyPhoneCard", zxApplyPhoneCard);
		model.addAttribute("zxcardepart", phoneApply.getXt_departinfo_name());
		return new ModelAndView("pc/zx-view/zx-apply-phone-card/zx-apply-phone-card-grant");
	}
}
