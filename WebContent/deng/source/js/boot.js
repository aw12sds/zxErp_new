...
function datatablesCallBack(data, callback, settings,url,opt){
	 ...
	 for (var i = 0; i < formdata.length; i++) {
		 paramdata[formdata[i]['name']] = encodeURI(formdata[i]['value']);
     };
...
function setDate(id,startDate,endDate){
	if(startDate != null){
	    $('#'+id).datepicker({maxDate:startDate});  
	}
	if(endDate != null){
		$(".form_datetime").datetimepicker({
			 format:"yyyy-mm-dd hh:ii:ss",
...
function clickrowselected(datatablesid){
	$('#'+datatablesid+' tbody').on('click', 'td', function() {
		$(this).parent().toggleClass('row_selected');
		if($(this).children( ".mt-checkbox" ).length == 0){
			if($(this).parent().hasClass("row_selected")){
				$(this).parent().find(".checkchild").prop("checked", true);
			}else{
				$(this).parent().find(".checkchild").prop("checked",false);
			}
		}
	});
}
...
function approval_record(id){
	...
	ajax:function (data, callback, settings){datatablesCallBack(data, callback, settings,'../lcApprovalController/getLcApprovalListByCondition?model_biz_id='+id,null);},//渲染数据
		//在第一位置追加序列号
...
//读取数据字典值，供Grid中使用
function InitBDataCallFnByKey(ckey,cvalue){
	var xt_data_dictionary_name="∨";
	$.ajax({
	   type:"GET",
	   ...
	   success: function(data){
		   if(data != ""){
			   $.each(data, function(i, item){
				   if(item.xt_data_dictionary_value == cvalue){
					   xt_data_dictionary_name = item.xt_data_dictionary_name;
				   }
	           })
		   }
	   }
