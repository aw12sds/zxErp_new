...
function updateXtDepartinfo() {
	submitBForm('defaultForm', '../xtDepartinfoController/updateXtDepartinfo',
			'../xtDepartinfoController/loadXtDepartinfo');
}
...
function departSelect() {
	...
	var setting = {
		...
		async : {
			...
			url : "../xtDepartinfoController/getXtDepartinfoBZTree",// Ajax获取数据的
																	// URL地址
			otherParam : {
				'expanded' : function() {
					return 'true'
...
function doDepartSelect() {
	...
	msgTishCallFnBoot("确定要选择【<font color=red>" + nodes[0].name + "</font>】？",
			function() {
